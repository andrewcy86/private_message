...
function rwpm_activate()
{
	...
	$query2 = 'CREATE TABLE IF NOT EXISTS ' . $wpdb->prefix . 'pm_users (
		`id` bigint(20) NOT NULL auto_increment,
		`pm_id` INT NOT NULL,
...
function rwpm_notify()
{
	...
	$num_unread = (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . $wpdb->prefix . 'pm a INNER JOIN ' . $wpdb->prefix . 'pm_users b ON a.id = b.pm_id WHERE b.recipient = "' . $current_user->ID . '" AND b.deleted != "2" AND b.viewed = 0' );
